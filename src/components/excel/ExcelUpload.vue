...
<script>
import { uploadMulFiles } from '@/api/excel'
...
export default {
  ...
  data() {
    return {
      ...
      uploadApis: {
        url: process.env.VUE_APP_BASE_API + process.env.VUE_APP_EXCEL_FILES_API || '', // 文件上传URL，默认为空，动态配置文件中获取
  ...
  methods: {
    ...
    getUploadUrl() {
      ...
      /*
      if (process.env.VUE_APP_BASE_API && process.env.VUE_APP_EXCEL_FILES_API) {
        return (process.env.VUE_APP_BASE_API + process.env.VUE_APP_EXCEL_FILES_API)
      } else {
